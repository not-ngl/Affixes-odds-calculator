...
window.ProbabilityCalculator = {
  ...
  calc2_1: function(Nr, Nu, Nc, upgrade, NrAsked, NuAsked, NcAsked) { // This function has been demonstrated to be true 
    ...
    const S1 = S - 1;
    const S2 = S - 2;
    const p = (Nr === 0) ? 0 : this.UPGRADE_CONFIG[upgrade].p;
    const p1 = (Nr === 1) ? 0 : this.UPGRADE_CONFIG[upgrade].p;
    const q = 1 - p;
    const q1 = 1 - p1;
  ...
  calc3_1: function(Nr, Nu, Nc, upgrade, NrAsked, NuAsked, NcAsked) { // This function has been demonstrated to be true
    ...
    const p = (Nr === 0) ? 0 : this.UPGRADE_CONFIG[upgrade].p;
    const p1 = (Nr === 1) ? 0 : this.UPGRADE_CONFIG[upgrade].p;
    const p2 = (Nr === 1) ? 0 : this.UPGRADE_CONFIG[upgrade].p;
    const q = 1 - p;
    const q1 = 1 - p1;
    const q2 = 1 - p2;
  ...
  calc3_3: function(Nr, Nu, Nc, upgrade, NrAsked, NuAsked, NcAsked) {
    ...
    const p = (Nr === 0) ? 0 : this.UPGRADE_CONFIG[upgrade].p;
    const p1 = (Nr === 1) ? 0 : this.UPGRADE_CONFIG[upgrade].p;
    const p2 = (Nr === 2) ? 0 : this.UPGRADE_CONFIG[upgrade].p;
    const q = 1 - p;
    const q1 = 1 - p1;
    const q2 = 1 - p2;
    ...
    if (upgrade === 3) {
      ...
      if (NrAsked === 2 && NcAsked === 1) return 2 * 2 * (p*p*q2 + p*p*q1 + p*p*q) / S / Nr / (Nr-1);
      ...
      if (NcAsked === 2 && NrAsked === 1) return 2 * 4 * (p*q1*q1 + p*q*q1 + p*q*q) / S / Nr / S2;
      if (NcAsked === 2 && NuAsked === 1) return 2 * 4 * q * q * q / S * ( 1/S2/S4 + 1/S2/S3 + 1/S1/S3 );
      if (NuAsked === 1 && NcAsked === 1 && NrAsked === 1) {
        return (p*q1*q1 + p*q*q1 + p*q*q) / Nr / S * ( 1/S1 + 1/S2 );
      }
    } else {
      if (NrAsked === 1) {
  ...
  calc4_1: function(Nr, Nu, Nc, upgrade, NrAsked, NuAsked, NcAsked) {
    ...
    const p1 = (Nr === 1) ? 0 : this.UPGRADE_CONFIG[upgrade].p;
    const p2 = (Nr === 2) ? 0 : this.UPGRADE_CONFIG[upgrade].p;
    const p3 = (Nr === 3) ? 0 : this.UPGRADE_CONFIG[upgrade].p;
    const q = 1 - p;
    const q1 = 1 - p1;
    const q2 = 1 - p2;
    const q3 = 1 - p3;
    ...
    if (upgrade === 3) {
      if (NrAsked === 1) {
        return p / Nr * (1 + p1 * (1 + p2 + p2*p3 + p2*q2 + q1 + p2*q1 + q1*q1) + q * (1 + p1 + p1*p2 + q1*p1 + q + q*p1 + q*q));
      }
    } else {
      if (NrAsked === 1) return p / Nr;
      if (NcAsked === 1) {
        const notR =   p *            ( Nu/S      * ( (Nu-1)/S1*S4/S2 + 2*(Nc-1)/S1*S5/S3 ) + 2*(Nc-1)/S  * ( Nu/S2*S5/S3     + 2*(Nc-2)/S2*S6/S4 ) );
        const notU =   q *   Nu / S * ( (Nu-1)/S1 * ( (Nu-2)/S2*S5/S3 + 2*(Nc-1)/S2*S6/S4 ) + 2*(Nc-1)/S1 * ( (Nu-1)/S3*S6/S4 + 2*(Nc-2)/S3*S7/S5 ) );
        const notC = 2*q * (Nc-1)/S * ( Nu/S2     * ( (Nu-1)/S3*S6/S4 + 2*(Nc-2)/S3*S7/S5 ) + 2*(Nc-2)/S2 * ( Nu/S4*S7/S5     + 2*(Nc-3)/S4*S8/S6 ) );
        return 1 - notR - notU - notC; 
      }
      if (NuAsked === 1) {
        const notR =   p *            ( (Nu-1)/S  * ( (Nu-2)/S1*S3/S2 +     2*Nc/S1*S4/S3 ) + 2*Nc/S      * ( (Nu-1)/S2*S4/S3 + 2*(Nc-1)/S2*S5/S4 ) );
        const notU =   q * (Nu-1)/S * ( (Nu-2)/S1 * ( (Nu-3)/S2*S4/S3 +     2*Nc/S2*S5/S4 ) + 2*Nc/S1     * ( (Nu-2)/S3*S5/S4 + 2*(Nc-1)/S3*S6/S5 ) );
        const notC = 2*q *   Nc / S * ( (Nu-1)/S2 * ( (Nu-2)/S3*S5/S4 + 2*(Nc-1)/S3*S6/S5 ) + 2*(Nc-1)/S2 * ( (Nu-1)/S4*S6/S5 + 2*(Nc-2)/S4*S7/S6 ) );
        return 1 - notR - notU - notC; 
      }
    }
    return 0;
